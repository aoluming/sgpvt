...
## InternVL dual-task (GA + KL) fork notes

This tree extends upstream XTuner with an experimental **InternVL** training path that combines **gradient ascent** with **attention-based image token perturbation and KL distillation** against a teacher. Key entry points:

- `xtuner/model/internvl.py` — model forward, attention mining, perturbation, and KL logic.
- `internvl_lora_finetune.py` — example config; override paths via `INTERNVL_MODEL_PATH`, `INTERNVL_ANN_PATH`, `INTERNVL_IMAGE_DIR`, `INTERNVL_TEACHER_PATH`, `INTERNVL_TARGET_ENTITY` (see comments in file).
- `examples/data/sample_annotations.json` and `examples/data/images/` — layout example; use basenames in JSON that resolve under your image root.

Install from the repo root (`pip install -e '.[all]'`), place InternVL weights and images, then run e.g. `xtuner train internvl_lora_finetune.py --deepspeed deepspeed_zero2`. DeepSpeed JSON presets live under `xtuner/configs/deepspeed/`. If `deepspeed_zero_config` is omitted in dual-task trainer args, the bundled `deepspeed_zero3.json` is used automatically.
